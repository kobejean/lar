...
TEST_F(SIFTTest, ParameterVariations) {
    ...
    std::vector<TestConfig> configs = {
        {"Default", 3, 0.04, 10, 1.6, CV_32F},
        {"Original_CV8U", 3, 0.02, 10, 1.6, CV_8U},  // Original parameters from Vision class
        {"High_Contrast", 3, 0.08, 10, 1.6, CV_32F}
    };
    
    for (const auto& config : configs) {
        lar::SIFTConfig sift_config(test_image.size());
        sift_config.nOctaveLayers = config.nOctaveLayers;
        sift_config.contrastThreshold = config.contrastThreshold;
        sift_config.edgeThreshold = config.edgeThreshold;
        sift_config.sigma = config.sigma;
        sift_config.descriptorType = config.descriptorType;

        lar::SIFT sift(sift_config);
