...
namespace lar {
  ...
  void ColmapRefiner::saveMap(std::string dir) {
    ...
    std::filesystem::path model_dir = std::filesystem::path(dir) / "colmap" / "sparse" / "0";
    colmap_db.writeSparseModel(
        model_dir.string(), data->frames, data->map.landmarks.all(),
        [this](size_t frame_id) { return data->getPathPrefix(frame_id).string() + "image.jpeg"; });
  }
